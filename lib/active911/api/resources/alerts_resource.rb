    ...
    class AlertsResource < Resource
      def show(alert_id:)
        ...
        begin
          ...
          raise Error, "Non JSON response received from Active911 (alerts/#{alert_id}) API: URL: #{result.env.url}, body: #{result.body}"
        end
        Alert.new(json_body)
      end
      ...
      def index(alert_days: nil, alert_minutes: nil)
        ...
        if alert_days
          raise Error, "alert_days must be between 1 and 30" if alert_days < 1 or alert_days > 30
          alert_params[:alert_days] = alert_days
        end

        if alert_minutes
          max_alert_minutes = 30 * 24 * 60
          raise Error, "alert_minutes must be between 1 and #{max_alert_minutes}" if alert_minutes < 1 or alert_minutes > max_alert_minutes
        ...
        begin
          ...
          raise Error, "Non JSON response received from Active911 (alerts) API: URL: #{result.env.url}, body: #{result.body}"
        end
